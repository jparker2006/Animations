...
void MainWindow::paintEvent(QPaintEvent *event) {
    ...
    QVector<QVector<float>> rotX = {
        {1, 0, 0},
        {0, (float) qCos(angle), (float) -qSin(angle)},
        {0, (float) qSin(angle), (float) qCos(angle)},
    };

    QVector<QVector<float>> rotY = {
        {(float) qCos(angle), 0, (float) -qSin(angle)},
        {0, 1, 0},
        {(float) qSin(angle), 0, (float) qCos(angle)},
    };

    QVector<QVector<float>> rotZ = {
        {(float) qCos(angle), (float) -qSin(angle), 0},
        {(float) qSin(angle), (float) qCos(angle), 0},
        {0, 0, 1}
    };
    ...
    for (int i=0; i<8; i++) {
        ...
        float distance = 1.4;
        float depth = 1 / (distance - rotated[2][0]);
